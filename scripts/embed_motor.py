...
workflow = {
    ...
    "nodes": [
        ...
        },
        {
            ...
            "parameters": {
                "operation": "executeQuery",
                # TODAS las citas confirmadas de hoy en adelante. El motor las
                # usa en JS para detectar traslapes con la hora que pida el
                # cliente (lenguaje natural). Sin parámetros.
                "query": "SELECT trabajador, to_char(fecha, 'YYYY-MM-DD') AS fecha_iso, to_char(hora_inicio, 'HH24:MI') AS ini, to_char(hora_fin, 'HH24:MI') AS fin FROM agenda.bookings WHERE estado = 'confirmada' AND fecha >= (now() AT TIME ZONE 'America/Mexico_City')::date;",
                "options": {},
            },
            "credentials": {"postgres": POSTGRES_CRED},
        },
        ...
        },
        {
            ...
            "parameters": {
                "jsCode": (
                    ...
                    "      + ((b.politicas ?? []).length ? `\\n\\nToma en cuenta:\\n${b.politicas.map(x => '- ' + x).join('\\n')}` : '')\n"
                    "      + `\\n\\nPara ver o cancelar tu cita escribe: mis citas. ¡Te esperamos!`,\n"
